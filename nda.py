...
class NDA(object):
    ...
    def assemble_group_bilinear_forms(self,g):
        ...
        self.sys_mats = sps.csc_matrix(A + B + C)
    ...
    def assemble_group_linear_forms(self,g):
        ...
        s = np.array(nodes)
        elem_alls = [] # list of elementary vector based on material
        ...
        for g in xrange(n_grp):
            for m in materials:
                ...
                # Retrieve cross-sections for cell
                sig_s = self.mat.get('sig_s', mat_id=m)[g]
                elem_s = sig_s*M
                elem_alls.append(elem_s)
            ...
            for cell in xrange(n):
                ...
                xx = 0
                for ii in mapping:
                    s[ii, 0] += elem_alls[m, xx]
                    xx += 1

            self.sys_rhses[g] = s + self.fixed_rhses[g]      
